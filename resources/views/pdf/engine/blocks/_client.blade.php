      ...
      <div style="border:1px solid #e5e7eb;border-left:3px solid {{ $primaryColor }};border-radius:3px;padding:9px 12px;background:#f9fafb;">
        ...
        @if($document->customer)
          ...
          <div style="font-size:8.5px;color:#374151;margin-top:6px;line-height:1.75;">
            ...
            @if(!empty($document->customer->city)){{ $document->customer->city }}@if(!empty($document->customer->postal_code)) – {{ $document->customer->postal_code }}@endif@if(!empty($document->customer->country)) · {{ $document->customer->country }}@endif<br>@endif
