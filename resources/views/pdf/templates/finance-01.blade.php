...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Serif, serif; font-size: 10px; color: #1A2B3C; margin: 0; background: #fff; }
    ...
    .security-bg { position: fixed; top: 38%; left: 8%; width: 84%; text-align: center;
        transform: rotate(-30deg); font-size: 30px; font-weight: bold;
        color: rgba(201,162,39,0.06); z-index: 0; letter-spacing: 8px; }
    header { position: fixed; top: -80px; left: 0; right: 0; height: 70px;
        background: #1E3A5F; padding: 0; border-bottom: 4px solid #C9A227; }
    .header-accent { background: #C9A227; height: 3px; }
    .header-inner { padding: 6px 14px 4px; }
    .bank-label { font-size: 7.5px; color: #C9A227; text-transform: uppercase; letter-spacing: 2px; }
    .company-name { font-size: 17px; font-weight: bold; color: #fff; letter-spacing: 1px; }
    .company-meta { font-size: 8px; color: #93C5FD; line-height: 1.5; }
    .doc-title { font-size: 14px; font-weight: bold; color: #C9A227; text-align: right; text-transform: uppercase; letter-spacing: 1.5px; }
    .doc-number { font-size: 10px; color: #BFDBFE; text-align: right; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #1E3A5F; border-top: 2px solid #C9A227; padding-top: 6px; background: #fff; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #C9A227; border-top: 3px solid #1E3A5F; padding: 10px 12px; background: #FEFCE8; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #1E3A5F; letter-spacing: 1px; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #1A2B3C; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #1E3A5F; font-style: italic; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #1E3A5F; color: #C9A227; padding: 7px 8px;
    ...
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #E0D9C0; font-size: 9.5px; }
    table.lines tbody tr:nth-child(even) td { background: #FAFAF5; }
    table.totals { width: 42%; margin-left: 58%; margin-top: 12px; border-collapse: collapse; }
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #E0D9C0; }
    table.totals .k { color: #1E3A5F; }
    table.totals .v { text-align: right; font-weight: bold; }
    table.totals tr.grand td { background: #1E3A5F; color: #C9A227; font-size: 12px; font-weight: bold; border: none; }
    ...
    .notes { font-size: 9px; color: #1A2B3C; padding-right: 20px; }
...
<div class="security-bg">DOCUMENT BANCAIRE SECURISE</div>
<header>
    ...
    <div class="header-inner">
        <table style="width:100%">
            <tr>
                <td style="width:55%">
                    <div class="bank-label">Établissement Bancaire</div>
                    ...
                    <div class="company-meta">
                        ...
                        @if($company->tax_id)<br>Agréé N° : {{ $company->tax_id }}@endif
                    </div>
                </td>
                <td style="width:45%">
...
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'Établissement agréé par les autorités de régulation bancaire.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
        ...
        <tr>
            <td>
                <table class="meta-table">
                    ...
                    @if ($document->reference)<tr><td class="k">N° Compte :</td><td>{{ $document->reference }}</td></tr>@endif
                    <tr><td class="k">Devise :</td><td>{{ $document->currency }}</td></tr>
                </table>
            </td>
            <td>
                    <div class="badge-box">
                        <div class="label">Titulaire / Client</div>
