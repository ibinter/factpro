...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Sans, sans-serif; font-size: 10px; color: #1A3A1A; margin: 0; background: #FFFBF0; }
    ...
    header { position: fixed; top: -75px; left: 0; right: 0; height: 65px;
        background: #15803D; padding: 8px 14px; border-bottom: 3px solid #FCD34D; }
    .company-name { font-size: 16px; font-weight: bold; color: #FCD34D; }
    .company-meta { font-size: 8px; color: #A7F3D0; line-height: 1.5; }
    .doc-title { font-size: 14px; font-weight: bold; color: #FEF3C7; text-align: right; text-transform: uppercase; }
    .doc-number { font-size: 10px; color: #FCD34D; text-align: right; }
    .mfi-tag { font-size: 7.5px; color: #BBF7D0; text-align: right; text-transform: uppercase; letter-spacing: 1px; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #15803D; border-top: 2px solid #FCD34D; padding-top: 6px; background: #FFFBF0; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #FCD34D; border-left: 4px solid #15803D; padding: 10px 12px; background: #F0FFF4; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #15803D; letter-spacing: 1px; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #1A3A1A; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #15803D; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #15803D; color: #FCD34D; padding: 7px 8px;
    ...
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #D1FAE5; font-size: 9.5px; }
    ...
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #D1FAE5; }
    table.totals .k { color: #15803D; }
    table.totals .v { text-align: right; font-weight: bold; }
    table.totals tr.grand td { background: #15803D; color: #FCD34D; font-size: 12px; font-weight: bold; border: none; }
    ...
    .notes { font-size: 9px; color: #1A3A1A; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #15803D; margin-bottom: 3px; }
    .hash { font-size: 6.5px; color: #9aa7b8; word-break: break-all; margin-top: 4px; }
    .uemoa-strip { background: #FCD34D; height: 4px; }
</style>
</head>
<body>
        ...
        <tr>
            <td style="width:55%">
                ...
                <div class="company-meta">
                    ...
                    @if($company->tax_id)<br>Agréé BCEAO N° : {{ $company->tax_id }}@endif
                </div>
            </td>
            <td style="width:45%">
                <div class="mfi-tag">Institution de Microfinance — UEMOA</div>
...
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'IMF agréée BCEAO — Au service de l\'inclusion financière en Afrique de l\'Ouest.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
            <td style="width:30%; text-align:right; color:#15803D;">Propulsé par <b>IBIG FactPro</b><br>factpro.ibigsoft.com</td>
        </tr>
    </table>
</footer>
<main>
    <div class="uemoa-strip"></div>
    <table class="addresses" style="margin-top:10px">
        <tr>
            <td>
                <table class="meta-table">
                    ...
                    @if ($document->reference)<tr><td class="k">N° Dossier :</td><td>{{ $document->reference }}</td></tr>@endif
                    <tr><td class="k">Devise :</td><td>{{ $document->currency }}</td></tr>
                </table>
            </td>
            <td>
                    <div class="badge-box">
                        <div class="label">Membre / Bénéficiaire</div>
                    @if ($document->customer)
                        <div class="name">{{ $document->customer->name }}</div>
                        <div style="font-size:8.5px;color:#166534;line-height:1.5">
    ...
    <table class="lines">
        <thead>
            <tr>
                <th style="width:42%">Désignation / Services</th>
