...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Serif, serif; font-size: 10px; color: #F1F5F9; margin: 0; background: #0F172A; }
    .watermark { position: fixed; top: 40%; left: 5%; width: 90%; text-align: center;
        transform: rotate(-35deg); font-size: 42px; font-weight: bold;
        color: rgba(251,191,36,0.12); z-index: 1000; letter-spacing: 4px; }
    header { position: fixed; top: -70px; left: 0; right: 0; height: 60px;
        background: #0F172A; padding: 8px 12px; border-bottom: 2px solid #FBBF24; }
    .company-name { font-size: 16px; font-weight: bold; color: #FBBF24; letter-spacing: 1px; }
    .company-meta { font-size: 8px; color: #94A3B8; line-height: 1.5; }
    .doc-title { font-size: 17px; font-weight: bold; color: #F1F5F9; text-align: right; text-transform: uppercase; letter-spacing: 3px; }
    .doc-number { font-size: 10px; color: #FBBF24; text-align: right; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #94A3B8; border-top: 1px solid #FBBF24; padding-top: 6px;
        background: #0F172A; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #334155; border-left: 3px solid #FBBF24; padding: 10px 12px; background: #1E293B; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #FBBF24; letter-spacing: 2px; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #F1F5F9; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #64748B; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #1E293B; color: #FBBF24; padding: 7px 8px;
    ...
    table.lines tbody td { padding: 8px 8px; border-bottom: 1px solid #1E293B; font-size: 9.5px; color: #CBD5E1; }
    table.lines tbody tr:nth-child(even) td { background: #1E293B; }
    table.totals { width: 42%; margin-left: 58%; margin-top: 12px; border-collapse: collapse; }
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #1E293B; }
    table.totals .k { color: #64748B; }
    table.totals .v { text-align: right; font-weight: bold; color: #F1F5F9; }
    table.totals tr.grand td { background: #FBBF24; color: #0F172A; font-size: 12px; font-weight: bold; border: none; }
    ...
    .notes { font-size: 9px; color: #94A3B8; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #FBBF24; margin-bottom: 3px; }
    .hash { font-size: 6.5px; color: #334155; word-break: break-all; margin-top: 4px; }
</style>
</head>
<body>
...
<footer>
    <table style="width:100%">
        <tr>
            ...
            <td style="width:30%; text-align:right;">Propulsé par <b style="color:#FBBF24">IBIG FactPro</b><br>factpro.ibigsoft.com</td>
        </tr>
    </table>
</footer>
<main>
        ...
        <tr>
            <td>
                <table class="meta-table">
                    <tr><td class="k">Date d'émission :</td><td style="color:#F1F5F9"><b>{{ $document->issue_date->format('d/m/Y') }}</b></td></tr>
                    @if ($document->due_date)<tr><td class="k">Échéance :</td><td style="color:#F1F5F9"><b>{{ $document->due_date->format('d/m/Y') }}</b></td></tr>@endif
                    @if ($document->reference)<tr><td class="k">Référence :</td><td style="color:#F1F5F9">{{ $document->reference }}</td></tr>@endif
                    <tr><td class="k">Devise :</td><td style="color:#F1F5F9">{{ $document->currency }}</td></tr>
                </table>
            </td>
            <td>
                    <div class="badge-box">
                    ...
                    @if ($document->customer)
                        ...
                        <div style="font-size:8.5px; color:#94A3B8; line-height:1.5">
