...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .company-name { font-size: 17px; font-weight: bold; color: #C9A227; letter-spacing: 1.5px; }
    .company-meta { font-size: 8px; color: #B0C4D8; line-height: 1.5; }
    .doc-title { font-size: 15px; font-weight: bold; color: #fff; text-align: right; text-transform: uppercase; letter-spacing: 2px; }
    .doc-number { font-size: 10px; color: #C9A227; text-align: right; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #1E3A5F;
        border-top: 2px solid #C9A227; border-bottom: 2px solid #1E3A5F;
        padding-top: 6px; background: #fff; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border-left: 4px solid #1E3A5F; border-top: 1px solid #C9A227; padding: 10px 12px; }
    ...
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #D1DCE8; font-size: 9.5px; }
    table.lines tbody tr:nth-child(even) td { background: #F0F4F8; }
    table.totals { width: 42%; margin-left: 58%; margin-top: 12px; border-collapse: collapse; }
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #D1DCE8; }
    ...
    .etude-stamp { font-size: 8px; color: #C9A227; text-align: right; font-style: italic; letter-spacing: 1px; }
</style>
</head>
<body>
...
<header>
    <table style="width:100%">
        <tr>
            <td style="width:55%">
                <div style="font-size:8px;color:#C9A227;letter-spacing:2px;text-transform:uppercase">Étude Notariale</div>
                ...
                <div class="company-meta">
                    ...
                    @if($company->tax_id)<br>CRPCEN N° : {{ $company->tax_id }}@endif
                </div>
            </td>
            <td style="width:45%">
                <div class="doc-title">{{ $document->type_label }}</div>
                <div class="doc-number">N° {{ $document->number }}</div>
                <div class="etude-stamp">Actes &amp; Formalités</div>
            </td>
        </tr>
    </table>
</header>
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'Étude habilitée — Actes reçus et conservés selon les dispositions légales.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
...
<main>
        ...
        <tr>
            ...
            <td>
                @if ($document->customer)
                    <div class="badge-box">
                        <div class="label">Partie comparante</div>
                        <div class="name">{{ $document->customer->name }}</div>
                        <div style="font-size:8.5px;color:#2C3E50;line-height:1.5">
    ...
    <table class="lines">
        <thead>
            <tr>
                <th style="width:42%">Désignation / Actes</th>
                <th class="num" style="width:10%">Qté</th>
                <th style="width:9%">Unité</th>
                <th class="num" style="width:14%">Émoluments HT</th>
