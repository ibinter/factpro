...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Sans, sans-serif; font-size: 10px; color: #1C1917; margin: 0; background: #fff; }
    ...
    header { position: fixed; top: -75px; left: 0; right: 0; height: 65px;
        background: #EA580C; padding: 8px 14px; }
    .company-name { font-size: 18px; font-weight: bold; color: #fff; text-transform: uppercase; letter-spacing: 1px; }
    .company-meta { font-size: 8px; color: #FED7AA; line-height: 1.5; }
    .doc-title { font-size: 16px; font-weight: bold; color: #fff; text-align: right; text-transform: uppercase; letter-spacing: 1px; }
    .doc-number { font-size: 10px; color: #FED7AA; text-align: right; }
    .coach-tag { font-size: 7.5px; color: #FEF3C7; text-align: right; text-transform: uppercase; letter-spacing: 2px; font-weight: bold; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #EA580C; border-top: 3px solid #EA580C; padding-top: 6px; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 2px solid #EA580C; padding: 10px 12px; background: #FFF7ED; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #EA580C; letter-spacing: 1px; font-weight: bold; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #1C1917; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #EA580C; font-weight: bold; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #111; color: #EA580C; padding: 7px 8px;
    ...
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #FED7AA; font-size: 9.5px; }
    ...
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #FED7AA; }
    table.totals .k { color: #78716C; }
    table.totals .v { text-align: right; font-weight: bold; }
    table.totals tr.grand td { background: #EA580C; color: #fff; font-size: 13px; font-weight: bold; border: none; }
    ...
    .notes { font-size: 9px; color: #1C1917; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #EA580C; margin-bottom: 3px; text-transform: uppercase; }
    .hash { font-size: 6.5px; color: #9aa7b8; word-break: break-all; margin-top: 4px; }
    .motivational { font-size: 9px; color: #EA580C; font-weight: bold; text-align: center; padding: 5px; border-top: 1px solid #FED7AA; margin-top: 5px; text-transform: uppercase; letter-spacing: 1px; }
</style>
</head>
<body>
...
<header>
    <table style="width:100%">
        <tr>
            ...
            <td style="width:45%">
                <div class="coach-tag">Coaching Personnel</div>
...
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'Votre coach personnel — Dépassez vos limites!' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
            <td style="width:30%; text-align:right; color:#EA580C; font-weight:bold;">IBIG FactPro<br><span style="color:#78716C;font-weight:normal">factpro.ibigsoft.com</span></td>
        </tr>
    </table>
</footer>
<main>
        ...
        <tr>
            <td>
                <table class="meta-table">
                    ...
                    @if ($document->reference)<tr><td class="k">Programme :</td><td>{{ $document->reference }}</td></tr>@endif
                    <tr><td class="k">Devise :</td><td>{{ $document->currency }}</td></tr>
                </table>
            </td>
            <td>
                    <div class="badge-box">
                        <div class="label">Coaché / Client</div>
                    @if ($document->customer)
                        <div class="name">{{ $document->customer->name }}</div>
                        <div style="font-size:8.5px;color:#57534E;line-height:1.5">
    ...
    <table class="lines">
        <thead>
            <tr>
                <th style="width:42%">Séance / Programme</th>
