...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #555; border-top: 3px solid #EAB308; padding-top: 6px; }
    .company-name { font-size: 18px; font-weight: bold; color: #111; }
    .company-meta { font-size: 8.5px; color: #555; line-height: 1.5; }
    .doc-title { font-size: 20px; font-weight: bold; color: #EAB308; text-align: right; text-transform: uppercase; }
    .doc-number { font-size: 11px; color: #555; text-align: right; }
    .tracking { background: #EAB308; color: #111; font-size: 9px; font-weight: bold;
        padding: 2px 10px; text-transform: uppercase; letter-spacing: 1px; display: inline-block; margin-top: 3px; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { background: #FEFCE8; border-left: 4px solid #EAB308; padding: 10px 12px; }
    ...
    table.lines thead th { background: #111; color: #EAB308; padding: 7px 8px; font-size: 9px;
    ...
    table.lines tbody tr:nth-child(even) td { background: #FEFCE8; }
    ...
    table.totals tr.grand td { background: #111; color: #EAB308; font-size: 12px; font-weight: bold; }
...
<header>
    <table style="width:100%"><tr>
        <td style="width:55%">
            ...
            <div class="tracking">&#9889; EXPRESS DELIVERY</div>
...
<footer>
    <table style="width:100%"><tr>
        <td style="width:70%">{{ $company->invoice_footer ?? 'Livraison rapide — satisfaction garantie.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
        <td style="width:30%; text-align:right;">Propulsé par <b style="color:#EAB308">IBIG FactPro</b><br>factpro.ibigsoft.com</td>
    </tr></table>
</footer>
<main>
    <table class="addresses"><tr>
        <td>
            <table class="meta-table">
                ...
                @if($document->reference)<tr><td class="k">Réf. / Code suivi :</td><td>{{ $document->reference }}</td></tr>@endif
