...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Serif, serif; font-size: 10px; color: #2D0F0F; margin: 0; background: #FEF7EC; }
    ...
    header { position: fixed; top: -80px; left: 0; right: 0; height: 70px;
        border-bottom: 3px double #B45309; padding: 8px 14px; background: #FEF7EC; }
    .cabinet-label { font-size: 8px; text-transform: uppercase; letter-spacing: 2px; color: #B45309; }
    .company-name { font-size: 17px; font-weight: bold; color: #7C2D12; letter-spacing: 1px; }
    .company-meta { font-size: 8px; color: #6B4C3B; line-height: 1.5; }
    .doc-title { font-size: 16px; font-weight: bold; color: #7C2D12; text-align: right; text-transform: uppercase; letter-spacing: 2px; }
    .doc-number { font-size: 10px; color: #B45309; text-align: right; font-style: italic; }
    .seal-line { border-top: 1px solid #B45309; border-bottom: 1px solid #B45309; margin: 6px 0; padding: 2px 0; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #7C2D12; border-top: 3px double #B45309; padding-top: 6px; background: #FEF7EC; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #B45309; padding: 10px 12px; background: #FFFAF5; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #7C2D12; letter-spacing: 1px; font-style: italic; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #2D0F0F; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #7C2D12; font-style: italic; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #7C2D12; color: #FEF7EC; padding: 7px 8px;
    ...
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #E8D0BC; font-size: 9.5px; }
    table.lines tbody tr:nth-child(even) td { background: #FFF8EF; }
    table.totals { width: 42%; margin-left: 58%; margin-top: 12px; border-collapse: collapse; }
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #E8D0BC; }
    table.totals .k { color: #7C2D12; font-style: italic; }
    table.totals .v { text-align: right; font-weight: bold; }
    table.totals tr.grand td { background: #7C2D12; color: #FEF7EC; font-size: 12px; font-weight: bold; border: none; }
    ...
    .qr-caption { font-size: 7px; color: #6B4C3B; margin-top: 2px; }
    .notes { font-size: 9px; color: #2D0F0F; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #7C2D12; margin-bottom: 3px; font-style: italic; }
...
<header>
    <table style="width:100%">
        <tr>
            <td style="width:55%">
                <div class="cabinet-label">Cabinet d'Avocats</div>
                ...
                <div class="company-meta">
                    ...
                    @if($company->tax_id)<br>Barreau N° : {{ $company->tax_id }}@endif
                </div>
            </td>
            <td style="width:45%; text-align:right">
                <div class="doc-title">{{ $document->type_label }}</div>
                <div class="doc-number">Réf. {{ $document->number }}</div>
                <div class="seal-line" style="text-align:right; font-size:7px; color:#B45309; letter-spacing:1px;">— DOCUMENT CONFIDENTIEL —</div>
            </td>
        </tr>
    </table>
</header>
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'Document établi conformément aux règles de déontologie du Barreau.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
            <td style="width:30%; text-align:right;">Propulsé par <b style="color:#7C2D12">IBIG FactPro</b><br>factpro.ibigsoft.com</td>
        </tr>
    </table>
</footer>
<main>
        ...
        <tr>
            <td>
                <table class="meta-table">
                    ...
                    @if ($document->reference)<tr><td class="k">Dossier N° :</td><td>{{ $document->reference }}</td></tr>@endif
            ...
            <td>
                    ...
                    @if ($document->customer)
                        ...
                        <div style="font-size:8.5px; color:#4A2C0A; line-height:1.5">
    ...
    <table class="lines">
        <thead>
            <tr>
                <th style="width:42%">Désignation / Diligences</th>
                <th class="num" style="width:10%">Qté</th>
                <th style="width:9%">Unité</th>
                <th class="num" style="width:14%">Honoraires HT</th>
    ...
    <table class="qr-section">
        <tr>
            <td class="notes">
                @if ($document->notes)<div class="title">Notes &amp; Observations</div><div>{!! nl2br(e($document->notes)) !!}</div>@endif
                @if ($document->terms)<div class="title" style="margin-top:8px">Conditions de règlement</div><div>{!! nl2br(e($document->terms)) !!}</div>@endif
            </td>
            <td class="qr-box">
