...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Sans, sans-serif; font-size: 10px; color: #3B1F5E; margin: 0; background: #FEF3C7; }
    ...
    header { position: fixed; top: -75px; left: 0; right: 0; height: 65px;
        background: #8B5CF6; padding: 8px 14px; border-bottom: 2px solid #DDD6FE; }
    .company-name { font-size: 16px; font-weight: bold; color: #FEF3C7; letter-spacing: 1px; }
    .company-meta { font-size: 8px; color: #DDD6FE; line-height: 1.5; }
    .doc-title { font-size: 14px; font-weight: bold; color: #FEF3C7; text-align: right; text-transform: uppercase; letter-spacing: 1px; }
    .doc-number { font-size: 10px; color: #C4B5FD; text-align: right; }
    .yoga-tag { font-size: 7.5px; color: #DDD6FE; text-align: right; text-transform: uppercase; letter-spacing: 2px; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #6D28D9; border-top: 1px solid #C4B5FD; padding-top: 6px; background: #FEF3C7; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #C4B5FD; padding: 10px 12px; background: #fff; border-radius: 4px; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #8B5CF6; letter-spacing: 1px; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #3B1F5E; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #8B5CF6; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #8B5CF6; color: #FEF3C7; padding: 7px 8px;
    ...
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #E9D5FF; font-size: 9.5px; color: #3B1F5E; }
    table.lines tbody tr:nth-child(even) td { background: #F5F3FF; }
    table.totals { width: 42%; margin-left: 58%; margin-top: 12px; border-collapse: collapse; }
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #E9D5FF; }
    table.totals .k { color: #7C3AED; }
    table.totals .v { text-align: right; font-weight: bold; color: #3B1F5E; }
    table.totals tr.grand td { background: #8B5CF6; color: #FEF3C7; font-size: 12px; font-weight: bold; border: none; }
    ...
    .qr-caption { font-size: 7px; color: #7C3AED; margin-top: 2px; }
    .notes { font-size: 9px; color: #3B1F5E; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #8B5CF6; margin-bottom: 3px; }
    .hash { font-size: 6.5px; color: #9aa7b8; word-break: break-all; margin-top: 4px; }
    .zen-line { text-align: center; font-size: 8px; color: #8B5CF6; letter-spacing: 3px; padding: 4px 0; border-top: 1px solid #E9D5FF; }
</style>
</head>
<body>
...
<header>
    <table style="width:100%">
        <tr>
            ...
            <td style="width:45%">
                <div class="yoga-tag">Yoga &amp; Bien-être · Sérénité</div>
...
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'Centre de bien-être — Harmonie du corps et de l\'esprit.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
            <td style="width:30%; text-align:right; color:#8B5CF6;">Propulsé par <b>IBIG FactPro</b><br>factpro.ibigsoft.com</td>
        </tr>
    </table>
</footer>
<main>
    <div class="zen-line">— Namaste —</div>
        ...
        <tr>
            <td>
                <table class="meta-table">
                    ...
                    @if ($document->reference)<tr><td class="k">Cours / Stage :</td><td>{{ $document->reference }}</td></tr>@endif
            ...
            <td>
                @if ($document->customer)
                    <div class="badge-box">
                        <div class="label">Pratiquant / Client</div>
                        <div class="name">{{ $document->customer->name }}</div>
                        <div style="font-size:8.5px;color:#6D28D9;line-height:1.5">
    ...
    <table class="lines">
        <thead>
            <tr>
                <th style="width:42%">Désignation / Cours</th>
