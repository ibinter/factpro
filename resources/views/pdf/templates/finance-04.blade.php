...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    header { position: fixed; top: -75px; left: 0; right: 0; height: 65px;
        background: #9B1C1C; padding: 8px 14px; border-bottom: 2px solid #6B7280; }
    .company-name { font-size: 16px; font-weight: bold; color: #fff; }
    .company-meta { font-size: 8px; color: #FCA5A5; line-height: 1.5; }
    .doc-title { font-size: 14px; font-weight: bold; color: #fff; text-align: right; text-transform: uppercase; }
    .doc-number { font-size: 10px; color: #FECACA; text-align: right; }
    .assur-tag { font-size: 7.5px; color: #FCA5A5; text-align: right; text-transform: uppercase; letter-spacing: 1px; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #6B7280; border-top: 2px solid #9B1C1C; padding-top: 6px; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #D1D5DB; border-left: 4px solid #9B1C1C; padding: 10px 12px; background: #FEF2F2; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #9B1C1C; letter-spacing: 1px; }
    ...
    .meta-table .k { color: #9B1C1C; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #9B1C1C; color: #fff; padding: 7px 8px;
    ...
    table.lines tbody tr:nth-child(even) td { background: #FEF2F2; }
    ...
    table.totals tr.grand td { background: #9B1C1C; color: #fff; font-size: 12px; font-weight: bold; border: none; }
    ...
    .notes .title { font-weight: bold; color: #9B1C1C; margin-bottom: 3px; }
        ...
        <tr>
            <td style="width:55%">
                ...
                <div class="company-meta">
                    ...
                    @if($company->tax_id)<br>N° CIMA : {{ $company->tax_id }}@endif
                </div>
            </td>
            <td style="width:45%">
                <div class="assur-tag">Compagnie d'Assurance</div>
...
<footer>
    <table style="width:100%">
        <tr>
            <td style="width:70%">{{ $company->invoice_footer ?? 'Assureur agréé — Votre protection, notre engagement.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
            <td style="width:30%; text-align:right; color:#9B1C1C;">Propulsé par <b>IBIG FactPro</b><br>factpro.ibigsoft.com</td>
        </tr>
    </table>
</footer>
<main>
        ...
        <tr>
            <td>
                <table class="meta-table">
                    ...
                    @if ($document->reference)<tr><td class="k">N° Police :</td><td>{{ $document->reference }}</td></tr>@endif
            ...
            <td>
                @if ($document->customer)
                    <div class="badge-box">
                        <div class="label">Assuré / Souscripteur</div>
    ...
    <table class="lines">
        <thead>
            <tr>
                <th style="width:42%">Désignation / Garanties</th>
                <th class="num" style="width:10%">Qté</th>
                <th style="width:9%">Unité</th>
                <th class="num" style="width:14%">Prime HT</th>
                <th class="num" style="width:8%">Rem. %</th>
                <th class="num" style="width:7%">Taxe %</th>
    ...
    <table class="totals">
        ...
        @endif
        <tr><td class="k">Taxes</td><td class="v">{{ number_format((float)$document->tax_amount,0,',',' ') }} {{ $document->currency }}</td></tr>
        <tr class="grand"><td>PRIME TTC</td><td class="v">{{ number_format((float)$document->total,0,',',' ') }} {{ $document->currency }}</td></tr>
    ...
    <table class="qr-section">
        <tr>
            <td class="notes">
                ...
                @if ($document->terms)<div class="title" style="margin-top:8px">Conditions générales</div><div>{!! nl2br(e($document->terms)) !!}</div>@endif
            </td>
            <td class="qr-box">
