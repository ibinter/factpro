...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Mono, DejaVu Sans, monospace; font-size: 10px; color: #e2e8f0; margin: 0; background: #111; }
    .watermark { position: fixed; top: 40%; left: 5%; width: 90%; text-align: center;
        transform: rotate(-35deg); font-size: 42px; font-weight: bold;
        color: rgba(34,197,94,0.15); z-index: 1000; letter-spacing: 4px; }
    header { position: fixed; top: -70px; left: 0; right: 0; height: 60px; background: #111; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #6B7280; border-top: 1px solid #22C55E; padding-top: 6px; background: #111; }
    .company-name { font-size: 16px; font-weight: bold; color: #22C55E; letter-spacing: 1px; }
    .company-meta { font-size: 8.5px; color: #6B7280; line-height: 1.5; }
    .doc-title { font-size: 18px; font-weight: bold; color: #22C55E; text-align: right; text-transform: uppercase; letter-spacing: 2px; }
    .doc-number { font-size: 11px; color: #6B7280; text-align: right; font-family: monospace; }
    .prompt { color: #22C55E; }
    .sector-badge { background: #374151; color: #22C55E; font-size: 8px; padding: 2px 8px;
        letter-spacing: 1px; display: inline-block; margin-top: 3px; font-family: monospace; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { background: #1f2937; border-left: 3px solid #22C55E; padding: 10px 12px; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #22C55E; letter-spacing: 1px; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #e2e8f0; margin: 3px 0; }
    .meta-table { margin-top: 8px; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #6B7280; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #374151; color: #22C55E; padding: 7px 8px; font-size: 9px;
        text-transform: uppercase; letter-spacing: 0.5px; text-align: left; }
    table.lines thead th.num, table.lines td.num { text-align: right; }
    table.lines tbody td { padding: 7px 8px; border-bottom: 1px solid #1f2937; font-size: 9.5px; color: #e2e8f0; }
    ...
    table.totals td { padding: 5px 8px; font-size: 10px; color: #e2e8f0; }
    table.totals .k { color: #6B7280; }
    table.totals .v { text-align: right; font-weight: bold; }
    table.totals tr.grand td { background: #374151; color: #22C55E; font-size: 12px; font-weight: bold; }
    table.totals tr.paid td { color: #4ade80; }
    ...
    .notes { font-size: 9px; color: #cbd5e1; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #22C55E; margin-bottom: 3px; font-size: 9px; }
    .hash { font-size: 6.5px; color: #374151; word-break: break-all; margin-top: 4px; }
</style>
</head>
<body>
...
<header>
    <table style="width:100%"><tr>
        <td style="width:55%">
            ...
            <div class="sector-badge">~# Freelance Dev — TJM</div>
            <div class="company-meta">
                ...
                @if($company->tax_id)<br>N° Fiscal / SIRET : {{ $company->tax_id }}@endif
            </div>
        </td>
        <td style="width:45%">
...
<footer>
    <table style="width:100%"><tr>
        <td style="width:70%">{{ $company->invoice_footer ?? 'Code livré — Paiement attendu dans les délais.' }}<br>Vérification : {{ $document->verificationUrl() }}</td>
        <td style="width:30%; text-align:right;">Propulsé par <b style="color:#22C55E">IBIG FactPro</b><br>factpro.ibigsoft.com</td>
    </tr></table>
</footer>
<main>
    <table class="addresses"><tr>
        <td>
            <table class="meta-table">
                ...
                @if($document->reference)<tr><td class="k">N° Mission / Sprint :</td><td>{{ $document->reference }}</td></tr>@endif
    ...
    <table class="lines">
        <thead><tr>
            <th style="width:42%">Mission / Développement</th>
            <th class="num" style="width:10%">Jours</th>
            <th style="width:9%">Unité</th>
            <th class="num" style="width:14%">TJM HT</th>
    ...
    <table class="totals">
        ...
        @if((float)$document->discount_amount>0)<tr><td class="k">Remise</td><td class="v">−{{ number_format((float)$document->discount_amount,0,',',' ') }} {{ $document->currency }}</td></tr>@endif
