...
<style>
    ...
    * { box-sizing: border-box; }
    body { font-family: DejaVu Sans, sans-serif; font-size: 10px; color: #2D1800; margin: 0; background: #FEF3C7; }
    .watermark { position: fixed; top: 40%; left: 5%; width: 90%; text-align: center;
        transform: rotate(-35deg); font-size: 42px; font-weight: bold;
        color: rgba(146,64,14,0.15); z-index: 1000; letter-spacing: 4px; }
    header { position: fixed; top: -70px; left: 0; right: 0; height: 60px;
        background: #92400E; padding: 8px 12px; border-bottom: 4px solid #166534; }
    .company-name { font-size: 16px; font-weight: bold; color: #FEF3C7; }
    .company-meta { font-size: 8px; color: #FDE68A; line-height: 1.5; }
    .doc-title { font-size: 17px; font-weight: bold; color: #FEF3C7; text-align: right; text-transform: uppercase; }
    .doc-number { font-size: 10px; color: #FDE68A; text-align: right; }
    footer { position: fixed; bottom: -70px; left: 0; right: 0; height: 60px;
        font-size: 8px; color: #555; border-top: 3px solid #92400E; padding-top: 6px;
        background: #FEF3C7; }
    .addresses { width: 100%; margin-bottom: 18px; }
    .addresses td { vertical-align: top; width: 50%; }
    .badge-box { border: 1px solid #92400E; border-left: 4px solid #166534; padding: 10px 12px; background: #FFFBEB; }
    .badge-box .label { font-size: 8px; text-transform: uppercase; color: #92400E; letter-spacing: 1.5px; }
    .badge-box .name { font-size: 12px; font-weight: bold; color: #2D1800; margin: 3px 0; }
    .meta-table td { padding: 2px 8px 2px 0; font-size: 9px; }
    .meta-table .k { color: #92400E; font-weight: bold; }
    table.lines { width: 100%; border-collapse: collapse; margin-top: 10px; }
    table.lines thead th { background: #92400E; color: #FEF3C7; padding: 7px 8px;
    ...
    table.lines tbody tr:nth-child(even) td { background: #FFFBEB; }
    table.totals { width: 42%; margin-left: 58%; margin-top: 12px; border-collapse: collapse; }
    table.totals td { padding: 5px 8px; font-size: 10px; border-bottom: 1px solid #FDE68A; }
    table.totals .k { color: #92400E; }
    table.totals .v { text-align: right; font-weight: bold; color: #2D1800; }
    table.totals tr.grand td { background: #92400E; color: #FEF3C7; font-size: 12px; font-weight: bold; border: none; }
    ...
    .notes { font-size: 9px; color: #44321A; padding-right: 20px; }
    .notes .title { font-weight: bold; color: #92400E; margin-bottom: 3px; }
    .hash { font-size: 6.5px; color: #FDE68A; word-break: break-all; margin-top: 4px; }
</style>
</head>
<body>
...
<footer>
    <table style="width:100%">
        <tr>
            ...
            <td style="width:30%; text-align:right;">Propulsé par <b style="color:#92400E">IBIG FactPro</b><br>factpro.ibigsoft.com</td>
        </tr>
    </table>
</footer>
<main>
        ...
        <tr>
            ...
            <td>
                    ...
                    @if ($document->customer)
                        ...
                        <div style="font-size:8.5px; color:#6B4A1A; line-height:1.5">
    ...
    <table class="totals">
        ...
        <tr class="grand"><td>TOTAL TTC / TOTAL</td><td class="v">{{ number_format((float)$document->total,0,',',' ') }} {{ $document->currency }}</td></tr>
        @if ((float)$document->amount_paid > 0)
            ...
            <tr class="due"><td class="k">Reste / Balance</td><td class="v">{{ number_format((float)$document->total-(float)$document->amount_paid,0,',',' ') }} {{ $document->currency }}</td></tr>
        @endif
    </table>
    <table class="qr-section">
        <tr>
            ...
            <td class="qr-box">
                ...
                <div class="qr-caption"><b>DOCUMENT AUTHENTIFIABLE</b><br>Scannez / Scan to verify</div>
