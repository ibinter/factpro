<template>
  <div class="widget-card">
    <h3 class="font-semibold text-gray-800 dark:text-white mb-4">Top 10 Clients</h3>
      ...
      <div v-for="(label, i) in data.labels" :key="i" class="flex items-center gap-3">
        ...
        <div class="flex-1">
          <div class="flex justify-between text-xs mb-0.5">
            <span class="text-gray-700 dark:text-gray-300 truncate max-w-[150px]">{{ label }}</span>
            <span class="font-semibold">{{ fmt(data.values[i]) }}</span>
          </div>
          <div class="h-1.5 bg-gray-100 dark:bg-gray-700 rounded-full">
            <div class="h-1.5 bg-blue-500 rounded-full" :style="{ width: pct(i) + '%' }" />
          </div>
        </div>
      </div>
    </div>
    <p v-else class="text-center text-gray-400 py-12 text-sm">Aucun client sur la période.</p>
  </div>
</template>
...
const { data, loading } = useWidgetData('top_clients', period)
